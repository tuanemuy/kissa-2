...
export async function adminListReports(
  ...
  Result<
    { items: ReportWithDetails[]; count: number },
    AdminReportManagementError
  >
> {
  try {
    ...
    const query: ListReportsQuery = {
      pagination: {
        ...
        limit: input.pagination?.size ?? 20,
        order: "desc",
        orderBy: "createdAt",
      },
      filter: input.filter,
    ...
    return ok(reportsResult.value);
  } catch (error) {
    return err(
      new AdminReportManagementError(
        "Unexpected error during report listing",
        ERROR_CODES.INTERNAL_ERROR,
        error,
      ),
    );
...
export async function reviewReport(
  ...
  try {
    ...
    const reportResult = await context.reportRepository.findById(
      input.reportId,
    );
    ...
    return err(
      new AdminReportManagementError(
        "Unexpected error during report review",
        ERROR_CODES.INTERNAL_ERROR,
        error,
      ),
    );
...
export async function getReportStatistics(
  ...
  try {
    ...
    return err(
      new AdminReportManagementError(
        "Unexpected error getting report statistics",
        ERROR_CODES.INTERNAL_ERROR,
        error,
      ),
    );
...
export async function markReportUnderReview(
  ...
  try {
    ...
    if (updateResult.isErr()) {
      return err(
        new AdminReportManagementError(
          "Failed to mark report under review",
          ERROR_CODES.INTERNAL_ERROR,
          updateResult.error,
        ),
      );
    ...
    return err(
      new AdminReportManagementError(
        "Unexpected error marking report under review",
        ERROR_CODES.INTERNAL_ERROR,
        error,
      ),
    );
...
export async function getEntityReports(
  ...
  try {
    ...
    const reportsResult = await context.reportRepository.findByEntity(
      entityType,
      entityId,
    );
    if (reportsResult.isErr()) {
    ...
    return err(
      new AdminReportManagementError(
        "Unexpected error getting entity reports",
        ERROR_CODES.INTERNAL_ERROR,
        error,
      ),
    );
