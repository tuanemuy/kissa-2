...
import { addPlaceToFavoritesAction } from "@/actions/favorites";
...
export function FavoriteButton({
  ...
  const [actionState, formAction, isPending] = useActionState(
    async () => await addPlaceToFavoritesAction(placeId),
  ...
  useEffect(() => {
    if (actionState.error && isPending === false) {
      if (actionState.error.message === "Place is already favorited") {
        toast.info("この場所は既にお気に入りに追加されています。");
      } else {
